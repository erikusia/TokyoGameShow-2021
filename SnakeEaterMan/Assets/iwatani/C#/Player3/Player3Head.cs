...
public class Player3Head : MonoBehaviour
{
    //食べた時
    [SerializeField]
    ...
    void Update()
    {
        if (hit == true)
        {
            ...
            gameObject.transform.root.GetComponent<Player3Move>().m_walk /= 2;
            gameObject.transform.root.GetComponent<Player3Move>().m_dash /= 2;
            if (hitCount >= 2)
            {
                hit = false;
            }
