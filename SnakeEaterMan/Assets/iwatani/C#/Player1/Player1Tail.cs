...
public class Player1Tail : MonoBehaviour
{
    //色変更用
    [SerializeField]
    ...
    void Update()
    {
        if (deathFlag == true)
        {
            ...
            PlayerPos.GetComponent<Player1Move>().enabled = false;
            ...
            if (respawnCount > 3)
            {
                ...
                deathFlag = false;
                PlayerPos.GetComponent<Player1Move>().enabled = true;
            }
        }
        else
        {
    ...
    {
        //頭がぶつかったら
        if (collision.gameObject.tag == "head2" || collision.gameObject.tag == "head3" ||
